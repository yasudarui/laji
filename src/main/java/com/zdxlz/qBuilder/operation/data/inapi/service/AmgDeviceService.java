...
@Service
public class AmgDeviceService  extends DataInCascadeTradeAbs {
    ...
    protected String bizProcess(CascadeReq cascadeReq) {
        ...
        if (BusinessType.INSERT.name().equals(operationType)) {
            AmgDevice amgDevice = MapUtil.get(map, "data", new TypeReference<AmgDevice>() {
            });
            amgDevice.setPlatformCode(platformCode);
            result = remoteAssetService.saveAmgDevice(amgDevice, SecurityConstants.INNER);
        } else if (BusinessType.UPDATE.name().equals(operationType)) {
            AmgDevice amgDevice = MapUtil.get(map, "data", new TypeReference<AmgDevice>() {
            });
            amgDevice.setPlatformCode(platformCode);
            result = remoteAssetService.updateAmgDevice(amgDevice, SecurityConstants.INNER);
            ...
            result = remoteAssetService.deleteAmgDevice(ids, SecurityConstants.INNER);
        } else if (BusinessType.IMPORT.name().equals(operationType)) {
            JSONArray deviceArray = JSONArray.parseArray(JSONObject.toJSONString(map.get("data")));
            for (int i = 0; i < deviceArray.size(); i++) {
                JSONObject deviceObj = deviceArray.getJSONObject(i);
                deviceObj.put("platformCode", platformCode);
            }
            result = remoteAssetService.importAmgDevice(ObjectMapperUtil.beanToJson(deviceArray), SecurityConstants.INNER);
        } else {
